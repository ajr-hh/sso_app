...
export default function ProfileScreen() {
  ...
  return (
    <ScrollView contentContainerStyle={styles.screen}>
      ...
      {error ? <ErrorBanner message={error} /> : null}
      {saved ? <Text style={styles.saved}>Profile and goals saved.</Text> : null}
      ...
      <EditableList
        ...
        onChange={(labels) => { setGoalLabels(labels); setSaved(false); }}
          ...
          try {
            await Promise.all([
              saveProfile({
                ...
                why_matters: profile.why_matters,
                motivators: profile.motivators,
                coach_style: profile.coach_style,
              }),
              replaceGoals(goalLabels.map((label) => label.trim()).filter(Boolean)),
            ]);
      ...
      <TaskSection
        ...
        onToggle={(task) => runTaskAction(() => toggleTask(task.id, !task.done))}
        tasks={tasks}
      />
    </ScrollView>
  );
  ...
  return (
    <Section title="About you">
      ...
      <Field label="Contact information">
        <TextInput
          onChangeText={(value) => update("contact_info", value)}
    ...
    <Section title="Today’s tasks">
      {tasks.map((task) => (
        <View key={task.id} style={styles.taskRow}>
          ...
          <Pressable accessibilityLabel={`Delete task: ${task.label}`} accessibilityRole="button" disabled={busy} onPress={() => onDelete(task.id)}>
...
function AddRow({ disabled, onAdd, onChange, placeholder, value }: {
  ...
  return (
    <View style={styles.row}>
      <TextInput onChangeText={onChange} onSubmitEditing={onAdd} placeholder={placeholder} style={[styles.input, styles.rowInput]} value={value} />
    ...
    <Section title={title}>
      {labels.map((label, index) => (
        <View key={rowKeys[index] ?? `new-${index}`} style={styles.row}>
          ...
          <Pressable accessibilityLabel={`Remove goal ${index + 1}`} accessibilityRole="button" onPress={() => onChange(labels.filter((_, itemIndex) => itemIndex !== index))}>
